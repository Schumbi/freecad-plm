...
@api_auth_required(post=ApiToken.Scope.CHECKOUT)
@require_http_methods(["POST"])
def checkout_add_file_api(request, checkout_id):
    checkout = get_object_or_404(
        Checkout.objects.select_related(
            "part",
            "part__project",
    ...
    if checkout.checked_out_by_id != request.user.id and not is_plm_admin(request.user):
        return JsonResponse(
            {"error": "Nur der Checkout-Besitzer darf Teile hinzufügen."},
            status=403,
        )
    revision_id = json_body(request).get("revision_id")
    revision = get_object_or_404(
        Revision.objects.select_related("part", "part__project"),
        id=revision_id,
    )
    try:
        addition = add_checkout_file(checkout, revision, request.user)
    except ValidationError as exc:
        return validation_error_response(exc, status=409)
    manifest = add_manifest_download_urls(checkout_manifest(checkout), request)
    added_entry = next(
        item for item in manifest["files"] if item["path"] == addition.path
    )
    return JsonResponse(
        {
            "checkout": checkout_payload(checkout),
            "added_file": added_entry,
            "manifest": manifest,
        },
        status=201,
    )
